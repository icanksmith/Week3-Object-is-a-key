...
objek-objek barang dari toko X tersebut yang berisikan info nama barang, 
...
function countProfit(shoppers) {
  ...
  for (let i = 0; i < listBarang.length; i++){
    ...
    for (let j = 0; j < shoppers.length; j++){
        if (shoppers[j].product === listBarang[i][0] && shoppers[j].amount <= objStock.leftOver){
            objStock.shoppers.push(shoppers[j].name);
            objStock.leftOver -= shoppers[j].amount;
            objStock.totalProfit += listBarang[i][1] * shoppers[j].amount;
        }
       
    }
    result.push(objStock);
  }
  return result;
}
